...
def spherical_bound_states(energy, m1, m2, a, V0, l):
    ...
    if l == 0:  # (l = 0) Solve with the analytical solution
        ...
        j_term = special.spherical_jn(l - 1, energy_j) / special.spherical_jn(
            l, energy_j)
        h_term = np.sqrt(-(m2 / m1) * (energy / (energy + V0))) * np.real(
            1j * (spherical_hankel(l - 1, energy_h, 1) /
                  spherical_hankel(l, energy_h, 1)))
...
class qd_results():
    ...
    def norm_wavefunction(self, l, n, **kwargs):
        ...
        wavefunction = np.concatenate([
            self.norm_A.iloc[n, l] *
            special.spherical_jn(l, r_in * self.k_in.iloc[n, l])**2,
            self.norm_B.iloc[n, l] * np.absolute(
                spherical_hankel(l, 1j * r_out * self.k_out.iloc[n, l],
                                 kind=1))**2
        ])
